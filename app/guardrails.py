...
def is_confident_enough(chunks: list[dict]) -> bool:
    """
    chunks: output of VectorStore.query(), each with a 'distance' field
    (cosine distance -- lower is more similar/confident).
    Returns False -> trigger the fallback tool instead of answering from the KB.
    """
    if not chunks:
        return False
